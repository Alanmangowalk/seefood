...
const RELEASE = '3.0-RC2.10.9.3-D1-SHADOW';
...
async function lineSessionLogin(env,p) {
  if(!env.SESSION_SECRET||!env.LINE_LOGIN_CHANNEL_ID)return {response:json({status:'config_required',code:'LINE_SESSION_NOT_CONFIGURED',msg:'D1 後台測試前必須先設定 LINE_LOGIN_CHANNEL_ID 與 SESSION_SECRET'},{status:503})};
...
async function buyItem(env,p,request) {
  ...
  const params={MerchantID:cfg.MerchantID,MerchantTradeNo:orderId,MerchantTradeDate:twDateTimeForEcpay(),PaymentType:'aio',TotalAmount:String(Math.round(total)),TradeDesc:'SEEFOOD_Order',ItemName:String(item.name||'SEEFOOD').slice(0,20),ReturnURL:`${new URL(request.url).origin}/api/ecpay/return`,ClientBackURL:`${new URL(request.url).origin}/?d1=1#orders`,ChoosePayment:String(p.payMethod)==='LINEPay'?'LINEPAY':'Credit',EncryptType:'1'};params.CheckMacValue=await ecpayMac(params,cfg.HashKey,cfg.HashIV);
...
async function queueInactiveShopReminders(env) {
  ...
  for(const x of candidates){
    ...
    await env.DB.batch([
      env.DB.prepare(`INSERT INTO notifications(notification_id,audience_type,user_id,shop_id,notification_type,title,body,deep_link,status,created_at) VALUES(?,'MERCHANT',?,?,'INACTIVE_7D',?,?,?,'UNREAD',?)`).bind(nid,x.owner_user_id,x.shop_id,title,body,'index.html?d1=1#merchant',now),
